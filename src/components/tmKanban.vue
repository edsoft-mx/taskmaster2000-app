...
function dropTargetIsValid(id, data = null) {
  ...
  return (
    (id.startsWith('main_') || id.startsWith('task_') || id.startsWith('epic_')) && differentState
  )
}
...
<template>
  <div class="row">
    <div
      ...
      :key="state.state"
      :class="`col stateColumn state_${state.state.replace(' ', '_')}`"
      @dragover="dragOverStateColumn($event)"
      @dragleave="dragLeaveStateColumn($event)"
    ...
    >
      <div class="stateTitle">
        {{ state.state }}
        <span
          ><small>{{ stateTasksLength.get(state) }} </small></span
        >
      </div>
      <div :class="`state_${state.state}`" :id="`container_${state.state.replace(' ', '_')}`">
        <div v-for="epic in epicsIndexed.get(state.state)" :key="epic.uiKey">
          <TMEpic
            ...
            @toggle-epic="$emit('toggle-epic', epic)"
            @select="$emit('select', epic)"
        ...
        <div v-for="task in tasksIndexed.get(state.state)" :key="task.uiKey">
          <TMTask
            ...
            :task="task"
            v-show="
              task.displayIndex < stateDisplayLimit.get(state.state) || task.ghost || task.justAdded
            "
