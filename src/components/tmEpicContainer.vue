...
function getClasses() {
  ...
  if (globalSelectedTask.value != null && globalSelectedTask.value?.idTask === props.epic.idTask) {
    result += ' selectedTask'
  }
  return result
}
...
<template>
  <div :class="getClasses()" :style="`border: solid 4px ${epic.color}`">
    ...
    <button
      @click="$emit('go-to-parent-epic', epic)"
    ...
    &nbsp;
    <b>{{ epic.key }}</b> - {{ epic.title }} <span class="state">{{ epic.state }}</span>
    &nbsp;
    <button
      @click="$emit('select', epic); toggleSuperTaskInfo()"
    ...
    <TMKanban
      ...
      @toggle-detail="
        (element) => {
          $emit('toggle-detail', element)
        }
      "
      @go-to-detail="
        (element) => {
          $emit('go-to-detail', element)
        }
      "
      @go-to-detail-epic="
        (element) => {
          $emit('go-to-detail-epic', element)
        }
      "
      @select="
        (subTask) => {
