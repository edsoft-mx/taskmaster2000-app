...
const isNewTask = computed(() => {
  let parts = props.hierarchy ? props.hierarchy.split('.') : ['0']
  return parts[parts.length - 1] === '0'
})

const isSubTask = computed(() => {
  let parts = props.hierarchy ? props.hierarchy.split('.') : []
  return parts.length > 1
})

const parentIsEpic = computed(() => {
  let parts = props.hierarchy ? props.hierarchy.split('.') : []
  if (parts.length > 1) {
    let parent = parts[parts.length - 2]
    return parent.startsWith('e')
  } else {
    return false
  }
})
...
//   let parts = props.hierarchy.split('.')
...
const nameOfSubTasks = computed(() => {
  let value = ''
  switch (formData.taskType.value) {
    case 'task':
      value = 'Subtasks'
      break
    case 'epic':
...
async function getData() {
  ...
  if (props.hierarchy !== '0') {
    //so, this is not a new task or epic at the root level, so let's get all the parents and itself
    formData.parentTasks = await callApi(
      'GET',
      `user/boards/${props.idBoard}/hierarchy/${props.hierarchy}`,
    )
    if (!isNewTask.value) {
      currentValues = formData.parentTasks[formData.parentTasks.length - 1]
      formData.parentTasks.splice(formData.parentTasks.length - 1, 1)
    }
  }
  if (isSubTask.value) {
    let delCount = 1
    if (!parentIsEpic.value) {
      delCount += 1
    }
    taskTypes.splice(0, delCount)

    for (let i = 0; i < boardProjects.value.length; i++) {
      let p = boardProjects.value[i]
      if (p.value !== formData.parentTasks[formData.parentTasks.length - 1].idProject) {
  ...
  if (isNewTask.value) {
    ...
    let defaultKind = !isSubTask.value || parentIsEpic.value ? 'task' : 'subtask'
    formData.taskType = taskTypes.find((tt) => tt.value === defaultKind)
    formData.project = boardProjects.value[0]
    if (!isSubTask.value) {
      title = 'New Task'
    } else {
      let parent = formData.parentTasks[formData.parentTasks.length - 1]
      formData.project = boardProjects.value.find((p) => p.value === parent.idProject)
      let kind = parent.taskType === 'epic' ? 'task' : 'subtask'
      title = `New ${kind} for ${parent.taskType}: ${parent.key}`
    }
    formData.state = boardStates.value.find((bs) => bs.value === searchParam('state'))
    //console.log('state')
    console.log(formData.state)
    formData.xtraData = {}
  } else {
    formData.idTask = currentValues.idTask
    ...
    let parent = formData.parentTasks[formData.parentTasks.length - 1]
    title = !isSubTask.value
      ? `Edit Task ${formData.key}`
      : `Edit ${parent.key}'s subtask ${formData.key}`
  }
  ...
  switch (isNewTask.value) {
    case true:
      if (formData.parentTask != null) {
        let parent = formData.parentTasks[formData.parentTasks.length - 1]
        data.idProject = parent.idProject
      }
...
function editTask(subtask, parentTask) {
  window.electronAPI.openTaskPage(
    `board/${props.idBoard}/task/${parentTask.idTask}/${subtask.idTask}`,
    null,
  )
}
...
function editTask2(subtask, cosa2) {
  editTask(subtask, cosa2)
  cosa2.expanded = true
}
      ...
      <q-card flat>
        <q-card-section>
          <div style="display: flex">
            ...
            <div style="width: 50%">
              <span v-for="(item, index) in formData.parentTasks" :key="item.idTask">
                <img :src="`${item.taskType}.png`" style="width: 16px; height: 16px" />
                {{ item.taskType }} - {{ item.key
                }}<span v-if="index < formData.parentTasks.length - 1"> &gt;️ </span>
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <div v-for="checklist in taskChecklists" :key="checklist.id">
              <div class="checklist">
                ...
                <q-option-group
                  v-model="checklist.completed"
                  :options="checklist.items"
                  type="checkbox"
                />
                <q-input
                  v-model="newCheckListItem"
                  label="New checklist item(s)"
                  autogrow
                  type="textarea"
                >
          ...
          <q-separator />
          <div
            v-if="formData.taskType.value === 'task' || formData.taskType.value === 'epic'"
            style="margin-top: 24px"
          >
            {{ nameOfSubTasks }}:
            <q-list bordered dense>
              <q-item
                v-for="subtask in formData.subTasks"
                :key="subtask.idTask"
                @click="editTask2(subtask, formData.taskRef)"
                :clickable="!subtask.isNew"
                :v-ripple="!subtask.isNew"
              >
                ...
                <q-item-section>
                  <q-item-label lines="2">
                    ...
                    <q-chip color="primary" dense square size="xs" text-color="white">{{
                      subtask.state
                    }}</q-chip>
                  </q-item-label>
                </q-item-section>
              ...
              <q-item v-if="formData.inEdition">
                <q-item-section avatar>
                  <q-avatar>
                    <img src="/subtask.png?url" style="width: 16px; height: 16px" />
                  </q-avatar>
                </q-item-section>
                <q-item-section>
                  <q-item-label>
                    <q-input
                      v-model="formData.newSubTaskTitle"
                      label="New Subtask or existing task key. (Press ENTER to add several at once)"
                      autogrow
                      type="textarea"
                    >
                      <template v-slot:after>
