  ...
  <q-layout view="hHh lpR fFf">
    <q-header elevated class="bg-primary text-white" id="mainHeader">
      <q-toolbar>
        ...
        <q-btn icon="hourglass_bottom" @click="pomodoroMenuClick" :label="remainingTime"> </q-btn>
        <q-space></q-space>
        ...
        <q-btn-dropdown dense icon="account_circle" :label="currentUsername" color="blue">
          <q-card style="min-width: 350px">
            ...
            <q-card-section class="q-pt-none">
              ...
              <q-input
                label="Messenger service host:port"
                name="messengerHostPort"
                v-model="messengerHostPort"
                autofocus
              ></q-input>
    ...
    <q-drawer v-model="rightDrawerOpen" side="right" bordered :width="rightDrawerWidth">
      <q-tab-panels v-model="rightDrawerTab" animated v-if="rightDrawerOpen">
        <q-tab-panel name="tabTodayTimeline">
          <TMTimeLine
            :include-breaks="timelineIncludeBreaks"
            :only-billed="false"
            uiId="tmRightSide"
            :end-date="today"
            :start-date="today"
          ...
          <q-list>
            ...
            >
              <div
                ...
                @dragover="dragOverQueue($event)"
                @dragleave="dragLeaveQueue($event)"
                @drop="dropTaskonQueue($event)"
              >
                ...
                <div v-for="task in taskQueue" :key="task.idTask">
                  <TMTask
                    class="task"
                    :task="task"
                    :id="`queue-task-card-${task.idTask}`"
                    @select="
                      (t) => {
                        onQueueTaskSelected(t)
                        openSearchResult(t)
                ...
                <q-btn-dropdown
                  color="primary"
                  label="Queue Actions"
                  :disable="queueSelectedTask == null"
                  auto-close
                >
                  <q-list>
            ...
            <q-expansion-item
              default-opened
              icon="task"
              :label="selectedTask ? selectedTask?.key : 'Task Info'"
              header-class="bg-primary text-white"
            >
              ...
              <q-card>
                <q-card-section>
                    ...
                    <q-item v-for="tag in tagsNoSystem" :key="tag.idTag" tag="label" v-ripple>
                      ...
                      <q-item-section side v-if="selectedTask != null && selectedTaskHasTag(tag)">
                        <q-btn
                          ...
                          dense
                          style="width: 18px"
                          @click="removeTagFromSelectedTask(tag)"
                        ></q-btn>
                      </q-item-section>
                    </q-item>
                  </q-list>
                  ...
                  <div v-if="filterByStartOrDue">
                    <q-date
                      v-model="selectedDaysStartOrDue"
                      range
                      today-btn
                      :events="getCalendarActivity"
                      :event-color="getCalendarActivityColor"
                    />
                  </div>
                  <br /><br />
                  <q-input
...
<script setup>
import { ref, reactive, provide, watch, onBeforeMount, onMounted, computed, nextTick } from 'vue'
import { callApi, callApiLogin, store_configuration } from 'src/common'
...
watch(filterOutOldFinishedTasks, async () => {
  if (loadComplete) {
    if (router.currentRoute.value.fullPath.startsWith('/MainLayout/boards/')) {
      boardExecuteOp.value = { op: 'refreshData' }
    }
...
async function applyDateRangeStartOrDueFilter() {
  if (loadComplete) {
    if (!filterByStartOrDue.value) {
      ...
      await callApi(
        'POST',
        'user/board_filters/show_start_or_due_days/',
        selectedDaysStartOrDue.value,
      )
    }
    if (router.currentRoute.value.fullPath.startsWith('/MainLayout/boards/')) {
      boardExecuteOp.value = { op: 'refreshData' }
...
function onQueueTaskSelected(task) {
  if (
    queueSelectedTask.value &&
    document.getElementById(`queue-task-card-${queueSelectedTask.value.idTask}`)
  ) {
    document
      .getElementById(`queue-task-card-${queueSelectedTask.value.idTask}`)
      .classList.remove('queueSelectedTask')
  }
